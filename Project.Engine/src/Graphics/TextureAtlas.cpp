...
namespace Graphics
{
        ...
        TextureAtlas::~TextureAtlas()
        {
                std::cout << "Cleared texture atlas\n";
        ...
        int TextureAtlas::Load(std::string path, int nbWidth, int nbHeight)
        {
                ...
                if(error)
                {
                        std::cerr << "Failed to open png file " << path << " : " << lodepng_error_text(error) << "\n";
                        return 1;
                }

                if((width%this->nbTileWidth))
                {
                        std::cerr << "Error when creating texture atlas " << path << ", NbWidth " << nbWidth << "is incorrect, must be a divisor of " << width << "\n";
                        return 2;
                }

                if((height%this->nbTileHeight))
                {
                        std::cerr << "Error when creating texture atlas " << path << ", NbWidth " << nbHeight << "is incorrect, must be a divisor of " << height << "\n";
                        return 2;
                }
                ...
                std::cout << "Atlas contains " << nbTileWidth * nbTileHeight << " using size " << tileWidth << "," << tileHeight << "\n";
                ...
                for(int tileY = 0; tileY < this->nbTileHeight; tileY++)
                {
                        for(int tileX = 0; tileX < this->nbTileWidth; tileX++)
                        {
                                ...
                                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, this->tileWidth, this->tileHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, &img[0]);
